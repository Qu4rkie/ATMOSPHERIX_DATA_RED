...
def prepare_to_likelihood(config,model,data,param):
    ...
    sig_v_int = 0.48   ### Half-width of the window [km/s] (we take half of SPIRou-1.14 (NIRPS-0.48) pixel)
    ...
    for i in range(config["num_transit"]):
        ...
        mask = np.where(data["window"][i]>0.2)[0]
        N_indiv = len(mask)
        
        tot_indiv = modfunc.total_model(param["Kp"],param["Vsys"],mod_2D,
                                        data["orders"][i], data["wmean"][i],
                                        data["V"][i],data["phase"][i],data["window"][i],
                                        data["Vstar"][i],ddv,data["proj"][i],
                                        config["emission"],model["flux_star"])
        ...
        for j in range(len(data["orders"][i])):
            ...
            indiv_data= indiv_data+(((data["intensity"][i][j][:,50:-50]).T-np.mean((data["intensity"][i][j][:,50:-50]).T,axis=0)).T)[mask].tolist() #The list format allows not to care about shapes
            indiv_std = indiv_std+(np.asarray([data["std"][i][j][50:-50]]*(N_indiv))).tolist() #duplicate Std end-start times
            #print(len(indiv_data))
            #number+=end-start
            number+=N_indiv
        #print("Length of Data :" + str(len(indiv_data)), flush=True)
        #print("Length of Model :" + str(len(indiv_model)), flush=True)
